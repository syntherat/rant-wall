...
export default function ProfilePage() {
  ...
  // load store items to map keys -> nice names
  useEffect(() => {
    let alive = true;
    ...
    async function loadStore() {
      try {
        ...
        const res = await api.storeItems();
        if (!alive) return;
        setStore(res.items || []);
      } catch {
        // if store fails, we’ll gracefully fallback to raw keys
      } finally {
        if (alive) setStoreLoading(false);
      }
    }

    // store is public — can load even if user is null, but your page redirects anyway
    loadStore();
  ...
  function nameForKey(key) {
    return storeMap.get(key)?.name || key;
  }

  // quick derived stuff (safe fallbacks)
  const equipped = user?.equipped || {};
  const themeKey = equipped.rantTheme || "theme.midnight";
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <span className="rounded-full border border-white/10 bg-black/40 px-3 py-1 text-[11px] text-white/70">
                ...
                <span className="text-white/90">
                  {storeLoading ? "Loading…" : nameForKey(themeKey)}
                </span>
              </span>
          ...
          <div className="mt-1 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <img
                src="/VELogo.png"
                alt="Vent Energy"
                className="h-7 w-7 opacity-90 animate-pulse drop-shadow-[0_0_8px_rgba(255,255,255,0.25)]"
              />
              <div className="text-2xl font-extrabold">{ve}</div>
            </div>
              ...
              {[
                ...
                >
                  ...
                  <div className="text-xs font-semibold text-white/85">
                    {storeLoading ? "Loading…" : nameForKey(key)}
                  </div>
                </div>
              ))}
            </div>
